#!/usr/bin/env python3
"""Cron publish wrapper for the New Mexico Wildfire Tracker.
...
def main():
    ...
    if not who.stdout.strip():
        run(["git", "config", "user.name", "nm-fire-bot"])
        run(["git", "config", "user.email", "nm-fire-bot@users.noreply.github.com"])
    ...
    print(f"🔥 New Mexico fires update — {s.get('fires')} active · {int(s.get('acres', 0)):,} ac · "
